...
locals {
  ...
  schema_tags                          = [for schema in var.apiary_managed_schemas : contains(keys(schema), "tags") ? merge(map("Name", "${local.apiary_bucket_prefix}-${replace(schema.schema_name, "_", "-")}"), var.apiary_tags, schema.tags) : merge(map("Name", "${local.apiary_bucket_prefix}-${replace(schema.schema_name, "_", "-")}"), var.apiary_tags)]
}
