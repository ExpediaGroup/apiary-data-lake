...
resource "aws_s3_bucket" "apiary_data_bucket" {
  ...
  lifecycle_rule {
    ...
    dynamic "transition" {
      ...
      for_each = lookup(each.value, "s3_lifecycle_policy_transition_period", var.s3_lifecycle_policy_transition_period) < lookup(each.value, "s3_object_expiration_days", 0) ? [1] : []
      content {
