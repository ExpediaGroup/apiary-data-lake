...
variable "lf_readonly_client_arns" {
  description = "AWS IAM role ARNs granted DESCRIBE,SELECT permissions on all glue databases and tables using LakeFormation."
  type        = list(string)
  default     = []
}
...
variable "datadog_metrics_hms_readwrite" {
  description = "HMS metrics to be sent to Datadog for Readwrite"
  type = list(
    object({
...
variable "datadog_metrics_hms_readonly" {
  description = "HMS metrics to be sent to Datadog for Readonly instance"
  type = list(
    object({
      name   = string # the original metric name from the Hive Metastore
      rename = optional(string) # the renamed metric
...
variable "datadog_metrics_hms_housekeeper" {
  description = "HMS metrics to be sent to Datadog for housekeeper instance"
  type = list(
    object({
      name   = string # the original metric name from the Hive Metastore
      rename = optional(string) # the renamed metric
