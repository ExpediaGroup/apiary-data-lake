...
resource "kubernetes_cron_job" "ranger_logs_repair" {
  count = (var.enable_ranger_logs && var.hms_instance_type == "k8s") ? 1 : 0
  metadata {
    name      = "${local.instance_alias}-ranger-logs-repair"
    namespace = "metastore"
  ...
  spec {
    concurrency_policy        = "Replace"
    failed_jobs_history_limit = 5
    schedule                  = var.ranger_logs_update_schedule
