...
resource "kubernetes_deployment_v1" "apiary_hms_readwrite_hive3" {
  count = var.hms_instance_type == "k8s" && var.hms_enable_hive3 ? 1 : 0
  metadata {
    name      = "${local.hms_alias}-readwrite-hive3"
  ...
  spec {
    ...
    template {
      metadata {
        ...
        annotations = {
          "ad.datadoghq.com/${local.hms_alias}-readwrite-hive3.check_names"  = var.datadog_metrics_enabled ? "[\"prometheus\"]" : null
          "ad.datadoghq.com/${local.hms_alias}-readwrite-hive3.init_configs" = var.datadog_metrics_enabled ? "[{}]" : null
          "ad.datadoghq.com/${local.hms_alias}-readwrite-hive3.instances"    = var.datadog_metrics_enabled ? "[{ \"prometheus_url\": \"http://%%host%%:${var.datadog_metrics_port}/actuator/prometheus\", \"namespace\": \"${var.hms_k8s_metrics_readwrite_namespace}\", \"metrics\": [ \"${join("\",\"", var.datadog_metrics_hms_readwrite_readonly)}\" ] , \"type_overrides\": { \"${join("\": \"gauge\",\"", var.datadog_metrics_hms_readwrite_readonly)}\": \"gauge\"} }]" : null
      ...
      spec {
        ...
        dynamic "init_container" {
          ...
          content {
            ...
            name  = "${local.hms_alias}-sql-init-readwrite-hive3"
...
resource "kubernetes_service" "hms_readwrite_hive3" {
  count = var.hms_instance_type == "k8s" && var.hms_enable_hive3 ? 1 : 0
  metadata {
    name      = "${local.hms_alias}-readwrite-hive3"
    namespace = var.metastore_namespace
