    ...
    template {
      metadata {
        ...
        annotations = {
          ...
          "ad.datadoghq.com/${local.hms_alias}-readwrite.instances"    = var.datadog_metrics_enabled ? jsonencode([
            {
              prometheus_url = "http://%%host%%:8080/actuator/prometheus"
              namespace      = "${var.hms_k8s_metrics_readwrite_namespace}"
              metrics        = local.hms_metrics
              type_overrides = local.hms_metrics_type_overrides
            }
          ]) : null
