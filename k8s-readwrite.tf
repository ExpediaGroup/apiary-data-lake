  ...
  spec {
    ...
    template {
      metadata {
        ...
        annotations = <<-EOF
          %{if ${var.datadog_metrics_enabled} == true}
            "ad.datadoghq.com/hms-readwrite.check_names": "[\"prometheus\"]"
            "ad.datadoghq.com/hms-readwrite.init_configs": "[{}]"
            "ad.datadoghq.com/hms-readwrite.instances": "[{ \"prometheus_url\": \"http://%%host%%:8080/actuator/prometheus\", \"namespace\": \"hms_readwrite\", \"metrics\": [ \"${join("\",\"", var.datadog_metrics_hms_readwrite_readonly)}\" ] , \"type_overrides\": { \"${join("\": \"gauge\",\"", var.datadog_metrics_hms_readwrite_readonly)}\": \"gauge\"} }]"
          %{endif}
